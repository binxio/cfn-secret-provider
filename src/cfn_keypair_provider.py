...
class KeyPairProvider(ResourceProvider):
    ...
    @property
    def arn(self):
        return "arn:aws:ec2:%s:%s:keypair/%s" % (
            self.region,
            self.account_id,
            self.get("Name"),
        )
    ...
    def import_keypair(self):
        try:
            self.ec2.import_key_pair(
                KeyName=self.get("Name"),
                PublicKeyMaterial=self.get("PublicKeyMaterial"),
            )
    ...
    def update(self):
        ...
        if key_name is None:
            self.fail(
                "could not get the key name from the physical resource id, %s"
    ...
    def delete(self):
        ...
        if key_name is not None:
            ...
            self.success(
                "key pair with the name %s is ignored" % self.physical_resource_id
            )

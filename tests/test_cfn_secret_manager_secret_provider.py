...
from cfn_secrets_manager_secret_provider import (
    SecretsManagerSecretProvider as SecretProvider,
)
...
def create_secret(binary=False):
    ...
    request = Request(
        "Create", name, secret={"msg": "het is hier geweldig!"}, binary_secret=binary
    )
    ...
    request = Request(
        "Update",
        name,
        secret={"msg": "het was hier geweldig!"},
        binary_secret=binary,
...
class Request(dict):
    def __init__(
        ...
        if binary_secret and secret:
            self["ResourceProperties"]["SecretBinary"] = b64encode(
                json.dumps(secret).encode()
            ).decode("ascii")

    def get_property(self, name):
        return (
            self["ResourceProperties"][name]
            if name in self["ResourceProperties"]
            else None
        )
